...
lib/engine/ # tool compilation, prompt formatting, IDE mappings
 ...
 lib/engine/mcp/ # MCP alias-map tool defs + injection helper (used by $browser/$browser-inject triggers, independent of dynamic-tools.js passthrough path)
  ...
  lib/engine/mcp/playwright.js → BROWSER_MCP alias map (alt naming) — same shape, aliases to mcp_playwright_browser_* tool names (browser_click/close/console_messages/drag/drop/evaluate/file_upload/fill_form/find/handle_dialog/hover/...)</parameter>
 ...
 lib/engine/triggers.js # exports { triggers, handleSkill } — triggers array of { triggers: string[], bpi?: string, params?: string[], passthrough?: bool, call?: (ctx) => string|void } entries: '$cwd' (pwd), '$save' (git diff), '$req' (call writes req.body JSON to temp/captures), '$grammar' (call writes dynamicGrammar txt to temp/captures), '$test' (full tool smoke-test), '$browser' (passthrough: true — injects BROWSER_MCP grammar from mcp/browser.js via injectMcpAliases, replaces literal '$browser' in message.content with `<mcp-browser>...</mcp-browser>`, request continues to provider), '$browser-inject' (passthrough: true — same tool registration but strips the trigger text instead of inlining grammar, i.e. enables tools silently without prompting the LLM this turn); handleSkill(skill, req, dynamicGrammar, parser) reads provider from parser.compiler.provider, invokes skill.call({ req, dynamicGrammar }) if defined, then parser.emitAndEnd(skill.bpi) — only used for non-passthrough (terminal) skills, since passthrough skills are consumed earlier in formatPrompt and never reach handleSkill; deterministic, no-reasoning first-step BPI text only, matched by ToolCompiler.matchSkill; params declares positional arg names substituted into #{name}# placeholders in bpi
