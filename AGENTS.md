...
lib/engine/ # tool compilation, prompt formatting, IDE mappings
 ...
 lib/engine/mcp/ # MCP alias-map tool defs + injection helper + auto-registration (used by $browser/$browser-inject triggers and auto-registered $<server> triggers) — dynamic-tools.js removed, this dir now owns MCP tool hashing/change-detection too
  lib/engine/mcp/auto.js → hashTools(tools) — sha256 of JSON.stringify(tools), used by registerAutoMcpServers (triggers.js) for session.dynamicToolsHash change detection; groupToolsByServer(tools) — regex mcp_<server>_<tool> groups req.body.tools[] by server name; buildParamSyntax(tool) — builds a bpi-syntax param fragment from JSON schema using SEP='\u00A6' internally (not the literal bpi param-separator char ¦) to avoid delimiter collision when building strings; buildAutoAliasMaps(tools) — returns { '$server': aliasMap } shaped identically to hand-written maps (e.g. BROWSER_MCP), using literal ⟦/
 ...
 lib/engine/triggers.js # exports { triggers, handleSkill, restoreMcpInjections, registerAutoMcpServers, matchMcpTrigger } — triggers array of { trigger: string, bpi?: string, params?: string[], passthrough?: bool, call?: (ctx) => string|void } (single `trigger` key, not an array) entries: '$cwd' (pwd), '$save' (git diff), '$req' (call writes req.body JSON to temp/captures), '$grammar' (call writes dynamicGrammar txt to temp/captures), '$test' (full tool smoke-test), '$browser' (passthrough: true — injects BROWSER_MCP grammar from mcp/browser.js via injectMcpAliases, strips the literal '$browser' text from the triggering message, then splices a new { role: 'internal', content: '<bpi_list title="browser tools">...</bpi_list>' } message immediately before it so the grammar is delivered as a system-authored message rather than embedded in user content; calls markMcpInjected(session, '$browser'), request continues to provider); handleSkill(skill, req, dynamicGrammar, parser) reads provider from parser.compiler.provider, invokes skill.call({ req, dynamicGrammar }) if defined, then parser.emitAndEnd(skill.bpi) — only used for non-passthrough (terminal) skills, since passthrough skills are consumed earlier in formatPrompt and never reach handleSkill; deterministic, no-reasoning first-step BPI text only, matched by ToolCompiler.matchSkill; params declares positional arg names substituted into #{name}# placeholders in bpi; MCP_ALIAS_MAPS = { $browser: BROWSER_MCP } registry maps a trigger tag to its alias map; MCP_ALIAS_PROBE_KEY precomputed once at load (tag → first bpiName in its map) used as a cheap already-registered probe; markMcpInjected(session, tag) sets session.mcpInjected[tag without leading $] = true (object map, not array); restoreMcpInjections(session, compilerTools, isNewSession) is a no-op unless isNewSession, then loops session.mcpInjected keys and, only if the probe key is missing from compilerTools (i.e. a fresh/restarted compiler), re-runs injectMcpAliases for that tag; registerAutoMcpServers(reqTools, session) hashes reqTools via hashTools (mcp/auto.js) for session.dynamicToolsHash change detection and merges any mcp_<server>_<tool>-named tools (or unmatched tools under a 'native' key) into MCP_ALIAS_MAPS without overwriting hand-written entries; matchMcpTrigger(word) synthesizes a passthrough skill on the fly for any tag in MCP_ALIAS_MAPS not in the static triggers array; '$mcp' (getter bpi listing Object.keys(MCP_ALIAS_MAPS)) and '$mcp-dump' (getter bpi returning JSON.stringify(MCP_ALIAS_MAPS)) triggers for runtime discovery — the old "so it doesn't re-log or re-register every message" behavior is preserved by the isNewSession gate now, so this note is redundant and removed
...
users.json (temp/users.json):
 {
   provider (deepseek|claude|chatgpt): {
     username: {
       ...
       sessions: array of { name, chatSessionId, parentMessageId, createdAt, lastUsed, disableTools, model, dynamicToolsHash?, _dynamicGrammarCache?, todos?, mcpInjected? (object map of MCP_ALIAS_MAPS tags injected via $browser/$<server>, keyed without the leading $, e.g. { browser: true }; restored into compiler.tools only on new session via restoreMcpInjections, survives server restart via users.json) },
...
- KNOWN LIMITATION: syncDynamicTools has an early return that always returns { changed: false, dynamicGrammar: '' } (grammar-string branch further below now also forced to ' ' on the alternate return), skipping cache/rebuild. Original dynamic grammar system (hash-cached, per-session grammar of MCP tools from req.body.tools) is dead code. MCP tool support is instead provided via lib/engine/mcp/ alias maps injected on-demand by the $browser/$browser-inject triggers, plus auto-registered $<server> triggers (see below).
- Auto MCP registration: registerAutoMcpServers(reqTools) (triggers.js) runs on every request inside syncDynamicTools, before its early return. It calls buildAutoAliasMaps (mcp/auto.js) to group any req.body.tools[] named mcp_<server>_<tool> by server, then merges each '$server' alias map into MCP_ALIAS_MAPS without overwriting existing hand-written entries (e.g. $browser). matchSkill (index.js) falls back to matchMcpTrigger(word) when word isn't a static skillsByTrigger entry but is a key in MCP_ALIAS_MAPS, synthesizing a passthrough skill on the fly via the same makePassthroughMcpCall factory $browser uses. All 3 routes call compiler.syncDynamicTools(...) before compiler.formatPrompt(...), so a request's own tools[] populate MCP_ALIAS_MAPS in time for that same request's skill matching.
...
- session.mcpInjected (object map, keyed without leading $, e.g. { browser: true }) tracks which MCP_ALIAS_MAPS tags have been injected via $browser/auto-registered $<server> triggers; restoreMcpInjections(session, compilerTools, isNewSession) only re-applies them when isNewSession is true (not on every formatPrompt call), so injected tool state survives server restart (in-memory ToolCompiler.objects cache is wiped on restart, but session persists via users.json) without redundant per-message work
- $mcp trigger lists all currently registered MCP_ALIAS_MAPS tags (available, not activated/missing) via a bpi getter evaluated fresh at emit time; $mcp-dump trigger returns the full MCP_ALIAS_MAPS object as JSON
