...
function syncDynamicTools(reqTools, session, compilerTools, reverseMap) {
  ...
  // Store hash on session for change detection.
  session.dynamicToolsHash = hash
  // Cache grammar on session for reuse when the tools hash is unchanged.
  // NOTE: this is written to users.json as-is by SessionSelector._saveUser
  ...
  // console.info(`[DynamicTools] Synced ${dynamicGrammar.length} ${grammarLines.length} MCP tool(s)`)
