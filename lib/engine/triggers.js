...
function markMcpInjected(session, tag) {
  ...
  session.mcpInjected[tag.slice(1)] = true
}
...
function registerAutoMcpServers(reqTools, session) {
  ...
  for (const [tag, aliasMap] of Object.entries(autoMaps)) {
    if (!MCP_ALIAS_MAPS[tag]) MCP_ALIAS_MAPS[tag] = aliasMap
  }
...
function restoreMcpInjections(session, compilerTools) {
  if (!session || !session.mcpInjected || typeof session.mcpInjected !== 'object') return
  for (const key of Object.keys(session.mcpInjected)) {
    const tag = `$${key}`
    const aliasMap = MCP_ALIAS_MAPS[tag]
    if (!aliasMap) continue

    // Skip if already registered (e.g. still-warm compiler) — only restore
    // when a fresh/restarted compiler is actually missing these tools. Static
    // maps use the precomputed probe key; auto-registered maps (only known at
    // runtime) fall back to computing it on the spot.
...
const triggers = [
  ...
  {
    trigger: '$mcp',
    // getter, not a static string — evaluated fresh each time handleSkill reads skill.bpi
    get bpi() {
      const tags = Object.keys(MCP_ALIAS_MAPS)
      return tags.length
        ? `Available MCP tags: ${tags.join(', ')}`
        : 'No MCP tags registered yet for this session — send a request with tools[] first, then retry.'
    },
  },
  {
    trigger: '$mcp-dump',
