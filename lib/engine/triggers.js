...
const triggers = [
  ...
  {
    triggers: ['$browser'],
    passthrough: true, // does not end the stream — inline-injected into the prompt, request continues to the provider
    call: ({ message, compilerTools }) => {
  ...
  {
    triggers: ['$browser-inject'],
    passthrough: true, // enables tools without sending the prompt to llm
    call: ({ message, compilerTools }) => {
      injectMcpAliases(BROWSER_MCP, compilerTools)
      message.content = message.content.replace(/\$browser-inject\s*/i, '')
    },
  },
  {
    triggers: ['$test'],
...
function handleSkill(skill, req, dynamicGrammar, parser) {
  ...
  console.info(
    `[${provider}] Skill trigger detected (${skill.triggers[0]}) - bypassing provider API`,
  )
  if (skill.call) {
...
module.exports = { triggers, handleSkill }
