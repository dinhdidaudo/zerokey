 ...
 * (the pipe here stands in for the real bpi param separator character).
 *
 * @param {object} tool
 * @returns {string}
 */
        ...
        def.type === 'integer' || def.type === 'number'
          ...
          : def.type === 'boolean'
            ...
            : def.type === 'array' || def.type === 'object'
              ...
              : 'str'
      const hint = `${key}={${type}}`
      return required.has(key) ? `${SEP}${hint}` : `(${SEP}${hint})?`
    })
    .join('')
}
...
function buildAutoAliasMaps(tools) {
  ...
  for (const [server, entries] of Object.entries(groups)) {
    ...
    for (const { rawName, bpiName, tool } of entries) {
      ...
      aliasMap[bpiName] = [rawName, BPI_OPEN + bpiName + paramSyntax + BPI_CLOSE + suffix]
    }
