...
const { restoreMcpInjections, matchMcpTrigger, registerAutoMcpServers } = require('./triggers')
...
class ToolCompiler {
  ...
  constructor(ideName, provider = 'chatgpt') {
    ...
    this._handlers = {
      ...
      tool: async (mes, messages, _is, uploadFile) => {
        ...
        if (name === 'view_image' || name === 'screenshot_page') {
          const files = decodeContentParts(mes.content)
  ...
  async formatPrompt(messages, isNewSession, uploadFile, session) {
    console.debug('[MESSAGES]', messages.length)
    restoreMcpInjections(session, this.tools)
    ...
    if (message?.role === 'user' && this._rawUser) {
      try {
        ...
        if (skill) console.info('[SKILL]', skill.trigger)
      } catch (e) {
    ...
    if (skill?.passthrough && typeof message.content === 'string') {
      ...
      skill.call({ messages, index: messages.length - 1, compilerTools: this.tools, session })
      skill = null
    }
  ...
  /**
   * Sync dynamic MCP tools from req.body.tools[]. Delegates to
   * registerAutoMcpServers (triggers.js) — the MCP layer owns both the
   * mcp_<server>_<tool> detection and the per-session hash/change-detection,
   * since it's the only thing that knows which tools belong to it.
   *
  ...
  syncDynamicTools(reqTools, session) {
    const { changed, hash } = registerAutoMcpServers(reqTools, session)
    return { changed, hash, dynamicGrammar: '' }
  }
