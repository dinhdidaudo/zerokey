...
const { syncDynamicTools } = require('./dynamic-tools')
...
class ToolCompiler {
  ...
  constructor(ideName, provider = 'chatgpt') {
    ...
    this._handlers = {
      ...
      user: async (mes, messages, isNewSession, uploadFile) => {
      ...
      tool: async (mes, messages, _is, uploadFile) => {
  ...
  async formatPrompt(messages, isNewSession, uploadFile) {
    ...
    if (message?.role === 'user' && this._rawUser) {
      try {
        const raw = this._rawUser(message.content, messages, isNewSession)
        const text = (typeof raw === 'string' ? raw : '').trim().toLowerCase()
        skill = ToolCompiler.matchSkill(text, raw)
      } catch (e) {
        console.debug('[SKILL-TRIGGER] rawUser check failed:', e.message)
      }
    }
    ...
    for (; i < messages.length; i++) {
      ...
      const result = handler
        ? await handler(mes, messages, isNewSession, uploadFile)
  ...
  buildPrompt(userPrompt, dynamicGrammar) {
    const base = instructions.getFull()
    const extra = dynamicGrammar || ''
    return `${base}${extra}\n\n${userPrompt}`
  }

  /**
   * Sync dynamic MCP tools from req.body.tools[].
   * Mutates this.tools in-place; stores only hash on session.
   *
   * @param {Array}  reqTools  req.body.tools[]
   * @param {object} session   current session
   * @returns {{ changed: boolean, hash: string, dynamicGrammar: string }}
   */
  syncDynamicTools(reqTools, session) {
    return syncDynamicTools(reqTools, session, this.tools, this._reverseMap)
  }
...
ToolCompiler.matchSkill = function (text, raw) {
  ...
  const skill = skillsByTrigger.get(word)
...
/**
 * Scan synthetic text through the Stream parser as tool_calls and end the
 * response — used to bypass the provider API and end the stream.
 *
 * @param {import('express').Response} res
 * @param {Stream} parser - already-constructed Stream for this response
 * @param {string} text - text to scan and emit
 */
ToolCompiler.emitAndEnd = function (res, parser, text) {
  parser.scan(text)
  parser.flush()
  parser.emit({}, 'stop', {})
  res.write('data: [DONE]\n\n')
